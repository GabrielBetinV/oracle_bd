...
-- Un método constructor es un método especial que se utiliza para crear e inicializar objetos de un tipo de objeto.
-- En PL/SQL, el método constructor se define utilizando la palabra clave CONSTRUCTOR en la declaración del método dentro del tipo de objeto.
-- El método constructor se llama automáticamente cuando se crea una nueva instancia del tipo de objeto, y se utiliza para asignar valores iniciales a los atributos del objeto. 
-- El método constructor puede aceptar parámetros que se utilizan para inicializar los atributos del objeto, lo que permite crear objetos con diferentes    valores iniciales según las necesidades del programa.

-- Ejemplo de un método constructor en un tipo de objeto
-- spec
create or replace TYPE PRODUCTO  AS OBJECT (    
--Atributos
codigo number,
...
-- Ver los tipos creados en la base de datos
...
-- La sobrecarga de métodos es una característica que permite definir múltiples métodos con el mismo nombre pero con diferentes listas de parámetros dentro de un tipo de objeto.
-- Esto permite que el mismo método pueda realizar diferentes operaciones dependiendo de los tipos y número de parámetros que se le pasen al método. En PL/SQL, la sobrecarga de métodos se
--  logra definiendo múltiples métodos con el mismo nombre pero con diferentes firmas (listas de parámetros) dentro del tipo de objeto.
...
-- Herencia
-- La herencia es un concepto fundamental en la programación orientada a objetos que permite crear nuevas clases (o tipos de objetos) basadas en clases existentes, heredando sus atributos y métodos.
-- En PL/SQL, la herencia se logra utilizando la palabra clave UNDER para crear un nuevo tipo de objeto que hereda de un tipo de objeto existente. El nuevo tipo de objeto puede
--  agregar nuevos atributos y métodos, así como sobrescribir los métodos heredados para proporcionar una implementación específica para el nuevo tipo de objeto. La herencia permite reutilizar código y crear una jerarquía de tipos de objetos que refleje las relaciones entre los conceptos del mundo real que se están modelando en la base de datos  .

-- Si es FINAL, no se puede heredar de ese tipo de objeto
-- Si es NOT FINAL, se puede heredar de ese tipo de objeto  
...
begin
  return SELF.precio - (precio * (impuestos/100));
end ver_precio;
...
-- Crear un nuevo tipo de objeto que hereda de PRODUCTO
...
declare
-- se debe colocar las variables del padre y luego la del hijo
...
begin
 ...
 -- Sobrescribir metodos de la clase padre
 -- En la clase hija, se puede proporcionar una nueva implementación para un método heredado de la clase padre utilizando la palabra clave OVERRIDING en la declaración del método dentro del tipo de objeto
-- En el ejemplo, se sobrescribe el método ver_producto en la clase comestibles para proporcionar una implementación específica que incluya la fecha de caducidad en la descripción del producto. Esto permite que el método ver_producto devuelva información más relevante para los objetos de tipo comestibles,
-- mientras que los objetos de tipo producto seguirán utilizando la implementación original del método ver_producto definida en la clase padre. La sobrescritura de métodos es una característica clave de la herencia que permite personalizar el comportamiento de los objetos en función de su tipo específico dentro de una jerarquía de clases.
...
-- Insertar datos en la tabla con columna de tipo objeto
INSERT INTO TIENDA VALUES (1, 1 ,PRODUCTO(1,'manzanas', 10));
INSERT INTO TIENDA VALUES (2,1, PRODUCTO(2,'naranjas', 20));
COMMIT;   

select * from tienda;

-- Para ver la informacion del objeto, con alias
select t.producto.precio from tienda t;

-- se puede ejecutar algunas funciones y objetos, dependiendo dela complejida
